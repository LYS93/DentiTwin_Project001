...
public class Player_Script : MonoBehaviour
{
        ...
        if (!isGoal)
        {
            if (Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.W)) // 위쪽 화살표 누를때 점프 && 2단점프 막기.
            {
                if (isJump == false)
                {
                    ...
                    isJump = true;
                    myAudio.PlayOneShot(myAudio_Clip[0]);
        ...
        if (collision.gameObject.CompareTag("Button"))
        {
            ...
            if (collision.gameObject.name == "Red Button") // 빨간버튼을 눌렀을 때 
            {
                ...
                if (!isTG_out) //가이드가 한번만 출력되게끔.
                {
                    ...
                    isTG_out = true;
                    myAudio.PlayOneShot(myAudio_Clip[1]);
                }
            }
            if (collision.gameObject.name == "Blue Button") // 파란버튼을 눌렀을 때
            {
                ...
                if (!isDrill_out)
                {
                    ...
                    myAudio.PlayOneShot(myAudio_Clip[2]);
            ...
            if (collision.gameObject.name == "Yellow Button") // 노란버튼을 눌렀을 때
            {
                ...
                if (!isImplant_out)
                {
                    ...
                    isImplant_out = true;
                    myAudio.PlayOneShot(myAudio_Clip[3]);
                }
    ...
    private void OnTriggerEnter2D(Collider2D collider)
    {
        if (collider.gameObject.name == "Ending")
        {
            ...
            isGoal = true;
            myAudio.PlayOneShot(myAudio_Clip[4]);
